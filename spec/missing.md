...
1. **Non-GitHub repos in reports** — `non_github_repo` status exists in the model but is not always populated by the adapters.
1. **SARIF output** — JSON model is SARIF-compatible but a dedicated `--format sarif` output writer is not implemented.
1. **SHA pinning / repo lock file** — SHAs are recorded per-scan but there is no `insecure-tree-repos.lock` file or `--repo-lock` flag.
1. **Dependency path enumeration** — `GraphEdge` records exist but the report does not yet show N shortest dependency paths per package.
